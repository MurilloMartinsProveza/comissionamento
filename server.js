// server.js (Versão Final com a rota /api/auth/me)
...
const VendaSchema = new mongoose.Schema({
    ...
    dadosVenda: { type: Object, required: true },
});
...
app.post('/api/auth/cadastro', async (req, res) => {
    try {
        ...
        if (!nome || !email || !senha) { return res.status(400).json({ msg: 'Por favor, preencha todos os campos.' }); }
        let usuario = await Usuario.findOne({ email });
        if (usuario) { return res.status(400).json({ msg: 'Este email já está cadastrado.' }); }
        const salt = await bcrypt.genSalt(10);
        const senhaCriptografada = await bcrypt.hash(senha, salt);
        usuario = new Usuario({ nome, email, senha: senhaCriptografada });
        await usuario.save();
        res.status(201).json({ msg: 'Usuário criado com sucesso!' });
    } catch (err) { console.error("ERRO na rota de cadastro:", err.message); res.status(500).send('Erro no servidor'); }
});

app.post('/api/auth/login', async (req, res) => {
    try {
        const { email, senha } = req.body;
        if (!email || !senha) { return res.status(400).json({ msg: 'Por favor, preencha todos os campos.' }); }
        const usuario = await Usuario.findOne({ email });
        if (!usuario) { return res.status(400).json({ msg: 'Email ou senha inválidos.' }); }
        const senhaCorreta = await bcrypt.compare(senha, usuario.senha);
        if (!senhaCorreta) { return res.status(400).json({ msg: 'Email ou senha inválidos.' }); }
        const payload = { usuario: { id: usuario.id } };
        const token = jwt.sign(payload, process.env.JWT_SECRET, { expiresIn: '8h' });
        res.json({ token });
    } catch (err) { console.error("ERRO na rota de login:", err.message); res.status(500).send('Erro no servidor'); }
});
...
const auth = (req, res, next) => {
    ...
    try {
        ...
        next();
    } catch (err) { res.status(401).json({ msg: 'Token inválido.' }); }
};
